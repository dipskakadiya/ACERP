...
<div>
    ...
    <script>
        $(document).ready(function() {
            $.pnotify({
                ...
                pnotify_text: 'JVENTRY Restore Successfully',
    ...
    <script>
        $(document).ready(function() {
            $.pnotify({
                pnotify_title: 'Success Notification',
                pnotify_text: 'JVENTRY Deleted Successfully',
                pnotify_animation: {effect_in: function(status, callback, pnotify){
                        var source_note = 'Always call the callback when the animation completes.';
                        var cur_angle = 0;
                        var cur_opacity_scale = 0;
...
<form method="post" name="formID" action="Trash.htm">
    ...
    <div class="mws-panel grid_8">
        <div class="mws-panel-header">
            <span class="mws-i-24 i-table-1">View All JVEntry</span>
        </div>
            ...
            <table class="mws-datatable-fn mws-table">
                ...
                <tbody>
                    ...
                    <tr class="gradeX">
                        ...
                        <td><% out.print(hmTrash.get("Bill_No"));%></td>
                        <td>          <% List ltAC = (List) request.getAttribute("dataAC");
                            HashMap hmAC = null;
                            if (ltAC != null) {
                                for (int j = 0; j < ltAC.size(); j++) {
                                    hmAC = (HashMap) ltAC.get(j);
                                    if (hmTrash.get("Dr_Ac").toString().equals(hmAC.get("Ac_Id").toString()) == true) {
                        ...
                        <td>          <% ltAC = (List) request.getAttribute("dataAC");
                            ...
                            if (ltAC != null) {
                                for (int j = 0; j < ltAC.size(); j++) {
                                    hmAC = (HashMap) ltAC.get(j);
                                    if (hmTrash.get("Cr_Ac").toString().equals(hmAC.get("Ac_Id").toString()) == true) {
                                        out.print(hmAC.get("Ac_Name"));
                                    }
                                }
                ...
                <div align="center" class="mws-button-row">
                    ...
                    %>
                    <input type="submit" class="mws-button save green" name="action" value="Restore_JVEntry" />
                    <%}%>
                    <% if (UST.equals("Admin") || (UST.equals("User") && (hper.get("P_Delete").toString().equals("1")))) {%>
                    <input type="submit" class="mws-button save green" name="action" value="Delete_JVEntry" />
                    <%}%>
