<%
    ...
    String Adminlist = "", Adminnode = "{";
    Adminlist+="'OAS("+ltAdmin.size()+")',";
    Adminnode+="'OAS("+ltAdmin.size()+")':{color:'red', shape:'dot', alpha:1},";
    if (ltAdmin != null) {
        for (int i = 0; i < ltAdmin.size(); i++) {
            hmAdmin = (HashMap) ltAdmin.get(i);
            Adminlist += "'" + hmAdmin.get("ADMIN_ID") +"("+hmAdmin.get("UCount") +")"+ "',";
            Adminnode += "'" + hmAdmin.get("ADMIN_ID") +"("+hmAdmin.get("UCount") +")"+ "':{color:'orange', shape:'dot', alpha:1},";
        }
        Adminlist = Adminlist.substring(0, Adminlist.length() - 1);
        Adminnode = Adminnode.substring(0, Adminnode.length() - 1);

    }
%>
<%
    ltAdmin = (List) request.getAttribute("DataUser");
    if (ltAdmin != null) {
        for (int i = 0; i < ltAdmin.size(); i++) {
            hmAdmin = (HashMap) ltAdmin.get(i);
            Adminnode += ",'" + hmAdmin.get("USER_ID")+ "':{color:'green', alpha:1}";
        }
        Adminnode += "}";
    }
%>
<%
    ltAdmin = (List) request.getAttribute("DataAdmin");
    List ltUser = (List) request.getAttribute("DataUser");
    HashMap hmUser = null;
    String joint = "{";
    if (ltAdmin != null) {
        joint += "'OAS("+ltAdmin.size()+")':{";
        for (int i = 0; i < ltAdmin.size(); i++) {
            hmAdmin = (HashMap) ltAdmin.get(i);
            joint += "'" + hmAdmin.get("ADMIN_ID")+"("+hmAdmin.get("UCOUNT") +")"+ "':{length:.8},";
        }
        joint = joint.substring(0, joint.length() - 1);
        joint += "},";
        for (int i = 0; i < ltAdmin.size(); i++) {
            hmAdmin = (HashMap) ltAdmin.get(i);
            joint += "'" + hmAdmin.get("ADMIN_ID")+"("+hmAdmin.get("UCOUNT") +")"+ "':{";
            if (ltUser != null) {
                for (int j = 0; j < ltUser.size(); j++) {
                    hmUser = (HashMap) ltUser.get(j);
                    if (hmUser.get("ADMIN_ID").toString().equals(hmAdmin.get("USER_COUNT").toString()) == true) {
                        joint += "'" + hmUser.get("USER_ID")+ "':{},";
                    }
                }
                if (joint.substring(joint.length() - 1, joint.length()).equals(",") == true) {
                    joint = joint.substring(0, joint.length() - 1);
                }
            }
            joint += "},";
...
<script>
    ...
    (function($){
                ...
                _initMouseHandling:function(){
                    ...
                    var handler = {
                        moved:function(e){
                            ...
                            }else if ($.inArray(nearest.node.name, [<%out.print(Adminlist);%>]) >=0 ){
        ...
        $(document).ready(function(){
            ...
            var theUI = {
                nodes: <%out.print(Adminnode);%>,
                edges: <%out.print(joint);%>
            }
...
<script type="text/javascript" src="plugins/arbor/mxh7kqd.js"></script>
...
<script src="plugins/arbor/jquery.address-1.4.min.js"></script>
<script src="plugins/arbor/arbor.js"></script>
<script src="plugins/arbor/arbor-tween.js"></script>
<script src="plugins/arbor/arbor-graphics.js"></script>
